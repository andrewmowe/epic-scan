...
function the_posts_navigation() {
	...
	<nav class="navigation posts-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Posts navigation', 'epic-scan' ); ?></h2>
		<div class="nav-links">

			<?php if ( get_next_posts_link() ) : ?>
			<div class="nav-previous"><?php next_posts_link( esc_html__( 'Older posts', 'epic-scan' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( esc_html__( 'Newer posts', 'epic-scan' ) ); ?></div>
			<?php endif; ?>
...
function the_post_navigation() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<h2 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'epic-scan' ); ?></h2>
		<div class="nav-links">
			<?php
...
function epic_scan_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'epic-scan' ),
...
function epic_scan_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && epic_scan_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'epic-scan' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'epic-scan' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'epic-scan' ) . '</span>', $tags_list ); // WPCS: XSS OK.
		}
	}

	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		echo '<span class="comments-link">';
		comments_popup_link( esc_html__( 'Leave a comment', 'epic-scan' ), esc_html__( '1 Comment', 'epic-scan' ), esc_html__( '% Comments', 'epic-scan' ) );
		echo '</span>';
	}

	edit_post_link( esc_html__( 'Edit', 'epic-scan' ), '<span class="edit-link">', '</span>' );
}
endif;
